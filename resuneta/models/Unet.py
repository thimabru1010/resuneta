...
class UNet(nn.HybridBlock):
    def __init__(self, num_classes, nfilter=64, **kwargs):
        ...
        with self.name_scope():
            ...
            # Decoder
            nfilter //= 2 #  512
            self.upconv6 = nn.Conv2D(nfilter, kernel_size=1, padding=0, use_bias=False, activation='relu')
            self.conv6_1 = nn.Conv2D(nfilter, kernel_size=3, padding=1)
            self.conv6_2 = nn.Conv2D(nfilter, kernel_size=3, padding=1)
            nfilter //= 2 #  256
            self.upconv7 = nn.Conv2D(nfilter, kernel_size=1, padding=0, use_bias=False, activation='relu')
            self.conv7_1 = nn.Conv2D(nfilter, kernel_size=3, padding=1)
            self.conv7_2 = nn.Conv2D(nfilter, kernel_size=3, padding=1)
            nfilter //= 2 #  128
            self.upconv8 = nn.Conv2D(nfilter, kernel_size=1, padding=0, use_bias=False, activation='relu')
            self.conv8_1 = nn.Conv2D(nfilter, kernel_size=3, padding=1)
            self.conv8_2 = nn.Conv2D(nfilter, kernel_size=3, padding=1)
            nfilter //= 2 #  64
            self.upconv9 = nn.Conv2D(nfilter, kernel_size=1, padding=0, use_bias=False, activation='relu')
